...
import { trackArchiveViewed, trackMixSelected } from "@/lib/analytics";
...
export default function ArchiveClient({ mixes }: { mixes: StoredMix[] }) {
  ...
  useEffect(() => {
    trackArchiveViewed({
      archive_item_count: mixes.length,
      archive_sort: "newest_first",
      archive_filter_direction: "all",
    });
  }, []);
  ...
  const filtered = mixes.filter((m) => {
    ...
    if (filter === "book→songs" || filter === "song→books")
      return m.kind === filter;
    if (filter === "this week")
      return now - m.createdAt < 7 * 24 * 60 * 60 * 1000;
    return true;
  });
  ...
  return (
    ...
    >
      ...
      <div className="vr-archive-pad" style={{ padding: "36px 56px 0" }}>
        ...
        >
          every mix anyone&apos;s made, in chronological order. links are
          permanent, anyone can listen. nothing is private here — that&apos;s
          the deal.
        </p>
        ...
        >
          {(["all", "book→songs", "song→books", "this week"] as Filter[]).map(
            (f) => (
              <button
                key={f}
                onClick={() => setFilter(f)}
      ...
      {filtered.length === 0 ? (
        ...
        >
          {filtered.map((m, i) => {
            ...
            const forLabel =
              m.kind === "book→songs" ? m.bookTitle : m.songTitle;
            const byLabel =
              m.kind === "book→songs" ? m.bookAuthor : m.songArtist;
            return (
              <a
                key={m.slug}
                href={`/r/${m.slug}`}
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    <span
                      style={{
                        ...
                        fontSize: 9,
                        letterSpacing: "0.1em",
                        color: m.kind === "song→books" ? P.blue : P.red,
                        textTransform: "uppercase",
                        padding: "2px 8px",
                        border: `1.5px solid ${m.kind === "song→books" ? P.blue : P.red}`,
                      }}
                    >
                      {m.kind}
                    </span>
                  ...
                  >
                    {m.kind === "song→books" ? "if you like " : "for "}
                    <em
                      style={{
                        color: P.ink,
                        fontStyle: "normal",
